...
// MatrixDrive Rev B ROM mux, Sega SMS mapper, and Codemasters SMS mapper.
...
module matrixdrive_mapper (
    ...
    // through A15:A13, covering the complete 64 KiB FRAM address space.
    wire fram_a13_active = codies_profile ?
                           codies_ram_bank[0] : cart_a[13];
    wire fram_a14_active = codies_profile ?
                           codies_ram_bank[1] : sega_control[2];
    ...
    wire fram_hi_ce_active = (sms_save_selected && fram_high_active) ?
                             1'b0 : 1'b1;
    wire fram_oe_active = sms_save_selected ? cas0_n : 1'b1;
    wire fram_we_active = sms_save_selected ? lwr_n : 1'b1;

    // External pulls hold memories inactive while these outputs are released
