...
**MatrixDrive Revision B** is an open hardware prototype for a USB-loadable Mega Drive/Genesis cartridge that can also run Master System software on compatible consoles and operate as the upper cartridge in Sonic & Knuckles lock-on mode. It combines an RP2350B installer, parallel NOR ROM, an instant-on CPLD mapper, and battery-free FRAM saves.
...
| --- | --- | --- |
| MD | Linear ROM / Sonic 2 lock-on | Linear ROM plus Sonic 3 save FRAM |
...
7. Insert MatrixDrive directly into the console or, for MD lock-on use, into the Sonic & Knuckles upper slot.
8. Insert Sonic & Knuckles into the console if used, then power on.

> [!WARNING]
> Never move SW2 or SW4 while powered. Never connect USB while MatrixDrive is inserted into a console or Sonic & Knuckles cartridge. Never hot-plug either cartridge.
...
| [`cpld/`](MatrixDrive_Dual_System_Cart_RevB/cpld/) | SMS mappers, MD lock-on save decoder, model, and RTL testbench |
| [`firmware/`](MatrixDrive_Dual_System_Cart_RevB/firmware/) | RP2350B/Pico SDK USB installer and lock-on ROM mirroring |
...
Only the Sonic 3 odd-byte MD save layout is implemented. Generic Mega Drive SRAM/EEPROM header auto-detection is not implemented. Korean, multicart, or game-specific SMS mappers, FM sound, light-gun support, and Sega mapper RAM-at-`$C000` mode are also outside this revision.
